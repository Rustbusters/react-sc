...
import { LogoDark, LogoLight } from './common/Logo';
...
const Sidebar = () => {
    ...
    return (
        <div className="hidden md:flex md:w-64 md:flex-col max-h-screen">
            <div className="flex flex-col flex-grow pt-5 overflow-y-auto bg-white dark:bg-gray-900">
                <div className="relative text-white w-64 min-h-screen p-4 ">
                    <LogoLight />
                    <LogoDark />

                    <div className="flex items-center flex-shrink-0 px-4">
                        <span className="text-md text-slate-500 dark:text-slate-300">Ilya's Rustbusters Server</span>
                    </div>
                    ...
                    <div className="flex flex-col flex-1 px-3 mt-6">
                        <div className="space-y-4">
                            <nav className="flex-1 space-y-2  dark:hover:bg-slate-800 rounded-lg">
                            ...
                            <nav className="flex-1 space-y-2 rounded-lg">
                                <button onClick={() => { setIsServersOpen(!isServersOpen); setCurPage("servers"); }} className="w-full text-start flex items-center px-4 py-2.5 text-sm font-medium transition-all duration-200 rounded-lg group text-slate-900 hover:bg-gray-100 dark:hover:bg-slate-800">
                                    <div className="flex space-x-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5 dark:text-white">
                                            <path strokeLinecap="round" strokeLinejoin="round" d="M5.25 14.25h13.5m-13.5 0a3 3 0 0 1-3-3m3 3a3 3 0 1 0 0 6h13.5a3 3 0 1 0 0-6m-16.5-3a3 3 0 0 1 3-3h13.5a3 3 0 0 1 3 3m-19.5 0a4.5 4.5 0 0 1 .9-2.7L5.737 5.1a3.375 3.375 0 0 1 2.7-1.35h7.126c1.062 0 2.062.5 2.7 1.35l2.587 3.45a4.5 4.5 0 0 1 .9 2.7m0 0a3 3 0 0 1-3 3m0 3h.008v.008h-.008v-.008Zm0-6h.008v.008h-.008v-.008Zm-3 6h.008v.008h-.008v-.008Zm0-6h.008v.008h-.008v-.008Z" />
                                        </svg>
                                        <span className="text-slate-900 dark:text-white">Servers</span>

                                    </div>
                                    <div className="flex flex-grow justify-end">
                                        {isServersOpen ? <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-5 dark:text-white">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 15.75 7.5-7.5 7.5 7.5" />
                                        </svg>
                                            : <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-5 dark:text-white">
                                    ...
                                    <div className="pl-5 space-y-1">
                                        {availableServers.map((id, index) => (
                                            <Link key={index} to={`/servers/${id}`} onClick={() => setCurPage(`servers/${id}`)} className={getStyles(`servers/${id}`)}>
                                                ...
                                                <span className="dark:text-gray-300">Server {id}</span>
                                            </Link>
                                        ))}
                                    </div>
                                )}
                            ...
                            <nav className="flex-1 space-y-2 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg">
